...
bool Scheduler::execute() {
    ...
    nrd |= _TASK_NEXTRUN_IMMEDIATE; // immediate
                ...
                // 修改复合赋値为分步赋値以解决C++20 volatile警告
                if ( iCurrent->iIterations > 0 ) {
                    long tempIterations = iCurrent->iIterations;
                    iCurrent->iIterations = tempIterations - 1;  // do not decrement (-1) being a signal of never-ending task
                }
                unsigned long tempRunCounter = iCurrent->iRunCounter;
                iCurrent->iRunCounter = tempRunCounter + 1;
#ifdef _TASK_SCHEDULING_OPTIONS
                switch (iCurrent->iOption) {
                  ...
                  case TASK_SCHEDULE_NC:
                    iCurrent->iPreviousMillis = iCurrent->iPreviousMillis + iCurrent->iDelay; 
                    {
                        ...
                        if ( ov < 0 ) {
                            long ii = i ? i : 1;
                            unsigned long tempPreviousMillis2 = iCurrent->iPreviousMillis;
                            iCurrent->iPreviousMillis = tempPreviousMillis2 + ((m - tempPreviousMillis2) / ii) * ii;
                        }
                    }
                    break;
                    
                  default:
                    unsigned long tempPreviousMillis1 = iCurrent->iPreviousMillis;
                    iCurrent->iPreviousMillis = tempPreviousMillis1 + iCurrent->iDelay;
                }
#else
                iCurrent->iPreviousMillis = iCurrent->iPreviousMillis + iCurrent->iDelay;
#endif  // _TASK_SCHEDULING_OPTIONS
...
#ifdef _TASK_TIMECRITICAL
        iCPUCycle = iCPUCycle + ( (micros() - tPassStart) - (tTaskFinish - tTaskStart) );
#endif  // _TASK_TIMECRITICAL
...
#ifdef _TASK_TIMECRITICAL
                iCPUIdle = iCPUIdle + (micros() - tIdleStart);
#endif  // _TASK_TIMECRITICAL
            }
        }
    }
